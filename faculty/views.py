...
def ViewProfs(request):
	...
	template=loader.get_template('prof1.html')
        context={'Courses':json.dumps(CourseList),'Coursedes':json.dumps(CoursedesList),'Prof_Name':request.user.username}
	...
	if request.method == 'POST':
		...
		if request.user.personnel.Role.Role_name=='Faculty':
			...
			for i in range(0,len(IC)):
				if person_id==IC[i].Inst_ID.Person_ID:
					CourseList.append(IC[i].Course_ID.Course_Name)						
	return render(request, 'forms.html', {'Courses':CourseList,'Prof_Name':request.user.username,'form': form,'request':request})
